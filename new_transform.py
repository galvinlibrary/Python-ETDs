...
root = 'C:\\Users\\tfluhr\\Documents\\islandora_cleanup\\DspaceUnproccessed\\etds_unzipped'
#location to place processed files.  Directory must be created beforehand
processed = 'C:\\Users\\tfluhr\\Documents\\islandora_cleanup\\DspaceProccessed\\etds'
...
def get_file_list(x):
    ...
    for dir in (x):
        ...
        move_path = (processed + '\\' + item_base + "_" + str(counter) + ".pdf")  # removed str(counter)
        xml_move_path = (processed + '\\' + item_base + "_" + str(counter) + '.xml')
        print(xml_move_path)
